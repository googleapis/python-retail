...
from google.cloud.retail_v2.services.user_event_service import UserEventServiceAsyncClient
...
from google.cloud.retail_v2.services.user_event_service.transports.base import _API_CORE_VERSION
from google.cloud.retail_v2.services.user_event_service.transports.base import _GOOGLE_AUTH_VERSION
...
def test_purge_user_events(transport: str = 'grpc', request_type=purge_config.PurgeUserEventsRequest):
    client = UserEventServiceClient(
        credentials=credentials.AnonymousCredentials(),
        transport=transport,
    )
...
def test_import_user_events(transport: str = 'grpc', request_type=import_config.ImportUserEventsRequest):
    client = UserEventServiceClient(
        credentials=credentials.AnonymousCredentials(),
        transport=transport,
    )
...
def test_rejoin_user_events(transport: str = 'grpc', request_type=user_event_service.RejoinUserEventsRequest):
    client = UserEventServiceClient(
        credentials=credentials.AnonymousCredentials(),
        transport=transport,
    )
...
def test_rejoin_user_events_from_dict():
...
def test_user_event_service_base_transport_with_credentials_file():
    ...
    with mock.patch.object(auth, 'load_credentials_from_file', autospec=True) as load_creds, mock.patch('google.cloud.retail_v2.services.user_event_service.transports.UserEventServiceTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
...
def test_user_event_service_base_transport_with_adc():
    ...
    with mock.patch.object(auth, 'default', autospec=True) as adc, mock.patch('google.cloud.retail_v2.services.user_event_service.transports.UserEventServiceTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
        adc.return_value = (credentials.AnonymousCredentials(), None)
        transport = transports.UserEventServiceTransport()
        adc.assert_called_once()
