...
from google.cloud.retail_v2.services.catalog_service.transports.base import _API_CORE_VERSION
from google.cloud.retail_v2.services.catalog_service.transports.base import _GOOGLE_AUTH_VERSION
...
def test_list_catalogs(transport: str = 'grpc', request_type=catalog_service.ListCatalogsRequest):
    client = CatalogServiceClient(
        credentials=credentials.AnonymousCredentials(),
        transport=transport,
    )
...
def test_list_catalogs_from_dict():
...
def test_update_catalog(transport: str = 'grpc', request_type=catalog_service.UpdateCatalogRequest):
    client = CatalogServiceClient(
        credentials=credentials.AnonymousCredentials(),
        transport=transport,
    )
...
def test_catalog_service_base_transport_with_credentials_file():
    ...
    with mock.patch.object(auth, 'load_credentials_from_file', autospec=True) as load_creds, mock.patch('google.cloud.retail_v2.services.catalog_service.transports.CatalogServiceTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
...
def test_catalog_service_base_transport_with_adc():
    ...
    with mock.patch.object(auth, 'default', autospec=True) as adc, mock.patch('google.cloud.retail_v2.services.catalog_service.transports.CatalogServiceTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
        adc.return_value = (credentials.AnonymousCredentials(), None)
        transport = transports.CatalogServiceTransport()
        adc.assert_called_once()
