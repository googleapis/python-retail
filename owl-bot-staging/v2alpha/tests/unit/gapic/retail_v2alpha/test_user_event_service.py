...
from google.cloud.retail_v2alpha.services.user_event_service import UserEventServiceAsyncClient
from google.cloud.retail_v2alpha.services.user_event_service import UserEventServiceClient
from google.cloud.retail_v2alpha.services.user_event_service import transports
...
def test_user_event_service_client_client_options_from_dict():
    with mock.patch('google.cloud.retail_v2alpha.services.user_event_service.transports.UserEventServiceGrpcTransport.__init__') as grpc_transport:
...
def test_rejoin_user_events(request_type, transport: str = 'grpc'):
    ...
    # Mock the actual call within the gRPC stub, and fake the request.
    with mock.patch.object(
            type(client.transport.rejoin_user_events),
            '__call__') as call:
        # Designate an appropriate return value for the call.
        call.return_value = operations_pb2.Operation(name='operations/spam')
...
@pytest.mark.asyncio
async def test_rejoin_user_events_async(transport: str = 'grpc_asyncio', request_type=user_event_service.RejoinUserEventsRequest):
    client = UserEventServiceAsyncClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport=transport,
    )
...
def test_rejoin_user_events_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, \
         mock.patch.object(path_template, "transcode")  as transcode, \
         mock.patch.object(operation.Operation, "_set_result_from_operation"), \
         mock.patch.object(transports.UserEventServiceRestInterceptor, "post_rejoin_user_events") as post, \
         mock.patch.object(transports.UserEventServiceRestInterceptor, "pre_rejoin_user_events") as pre:
        pre.assert_not_called()
        post.assert_not_called()
...
def test_user_event_service_base_transport():
    ...
    with mock.patch('google.cloud.retail_v2alpha.services.user_event_service.transports.UserEventServiceTransport.__init__') as Transport:
...
def test_user_event_service_base_transport_with_credentials_file():
    ...
    with mock.patch.object(google.auth, 'load_credentials_from_file', autospec=True) as load_creds, mock.patch('google.cloud.retail_v2alpha.services.user_event_service.transports.UserEventServiceTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
...
def test_user_event_service_base_transport_with_adc():
    ...
    with mock.patch.object(google.auth, 'default', autospec=True) as adc, mock.patch('google.cloud.retail_v2alpha.services.user_event_service.transports.UserEventServiceTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
        adc.return_value = (ga_credentials.AnonymousCredentials(), None)
        transport = transports.UserEventServiceTransport()
        adc.assert_called_once()
