...
class SearchServiceClient(metaclass=SearchServiceClientMeta):
    ...
    @staticmethod
    def experiment_path(project: str,location: str,catalog: str,experiment: str,) -> str:
        """Returns a fully-qualified experiment string."""
        return "projects/{project}/locations/{location}/catalogs/{catalog}/experiments/{experiment}".format(project=project, location=location, catalog=catalog, experiment=experiment, )
    ...
    @staticmethod
    def serving_config_path(project: str,location: str,catalog: str,serving_config: str,) -> str:
        """Returns a fully-qualified serving_config string."""
        return "projects/{project}/locations/{location}/catalogs/{catalog}/servingConfigs/{serving_config}".format(project=project, location=location, catalog=catalog, serving_config=serving_config, )

    @staticmethod
    def parse_serving_config_path(path: str) -> Dict[str,str]:
        """Parses a serving_config path into its component segments."""
        m = re.match(r"^projects/(?P<project>.+?)/locations/(?P<location>.+?)/catalogs/(?P<catalog>.+?)/servingConfigs/(?P<serving_config>.+?)$", path)
        return m.groupdict() if m else {}
    ...
    def search(self,
        ...
        Args:
            request (Union[google.cloud.retail_v2alpha.types.SearchRequest, dict]):
