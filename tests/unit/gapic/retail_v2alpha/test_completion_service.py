...
from google.cloud.retail_v2alpha.services.completion_service import (
...
def test_completion_service_client_client_options_from_dict():
    with mock.patch(
        "google.cloud.retail_v2alpha.services.completion_service.transports.CompletionServiceGrpcTransport.__init__"
    ) as grpc_transport:
...
def test_import_completion_data_rest_required_fields(
    request_type=import_config.ImportCompletionDataRequest,
):
    transport_class = transports.CompletionServiceRestTransport
    ...
    unset_fields = transport_class(
        credentials=ga_credentials.AnonymousCredentials()
    ).import_completion_data._get_unset_required_fields(jsonified_request)
    jsonified_request.update(unset_fields)
...
def test_import_completion_data_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(
        ...
        transports.CompletionServiceRestInterceptor, "post_import_completion_data"
    ) as post, mock.patch.object(
        transports.CompletionServiceRestInterceptor, "pre_import_completion_data"
    ) as pre:
        pre.assert_not_called()
        post.assert_not_called()
        pb_message = import_config.ImportCompletionDataRequest.pb(
            import_config.ImportCompletionDataRequest()
        )
        transcode.return_value = {
            "method": "post",
...
def test_import_completion_data_rest_bad_request(
    transport: str = "rest", request_type=import_config.ImportCompletionDataRequest
):
    client = CompletionServiceClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport=transport,
...
def test_completion_service_base_transport():
    ...
    with mock.patch(
        "google.cloud.retail_v2alpha.services.completion_service.transports.CompletionServiceTransport.__init__"
    ) as Transport:
...
def test_completion_service_base_transport_with_credentials_file():
    ...
    with mock.patch.object(
        ...
        "google.cloud.retail_v2alpha.services.completion_service.transports.CompletionServiceTransport._prep_wrapped_messages"
    ) as Transport:
        Transport.return_value = None
